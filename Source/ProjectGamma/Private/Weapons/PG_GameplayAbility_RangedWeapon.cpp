...
namespace PGConsoleVariables
{
	...
	static FAutoConsoleVariableRef CVarDrawBulletHits(
		TEXT("pg.Weapon.DrawBulletHitDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHitRadius(
		TEXT("pg.Weapon.DrawBulletHitRadius"),
		DrawBulletHitRadius,
...
UPG_GameplayAbility_RangedWeapon::UPG_GameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

UPG_RangedWeaponInstance* UPG_GameplayAbility_RangedWeapon::GetWeaponInstance() const
{
	return Cast<UPG_RangedWeaponInstance>(GetAssociatedEquipment());
}

bool UPG_GameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle,
	...
	if (bResult)
	{
		if (GetWeaponInstance() == nullptr)
		{
			UE_LOG(LogTemp, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				...
				*UPG_RangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
void UPG_GameplayAbility_RangedWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle,
	...
	const FGameplayEventData* TriggerEventData)
{
	...
	UPG_RangedWeaponInstance* WeaponData = GetWeaponInstance();
	check(WeaponData);
...
void UPG_GameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle,
	const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo,
...
int32 UPG_GameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
FHitResult UPG_GameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace,
...
FHitResult UPG_GameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace,
	float SweepRadius, bool bIsSimulated, TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (PGConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, PGConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void UPG_GameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData,
	TArray<FHitResult>& OutHits)
{
	UPG_RangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (PGConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, PGConsoleVariables::DrawBulletHitRadius, FColor::Red, false, PGConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void UPG_GameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel UPG_GameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams,
	bool bIsSimulated) const
{
	return PG_TraceChannel_Weapon;
}

void UPG_GameplayAbility_RangedWeapon::PerformLocalTargeting(TArray<FHitResult>& OutHits)
{
	...
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, EPG_AbilityTargetingSource::CameraTowardsFocus);
		...
		if (PGConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, PGConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
FVector UPG_GameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform UPG_GameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn,
	EPG_AbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController(); 
	UPG_WeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<UPG_WeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != EPG_AbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == EPG_AbilityTargetingSource::CameraTowardsFocus) || (Source == EPG_AbilityTargetingSource::PawnTowardsFocus) || (Source == EPG_AbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == EPG_AbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == EPG_AbilityTargetingSource::WeaponForward) || (Source == EPG_AbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else
	...
	if (bFoundFocus && ((Source == EPG_AbilityTargetingSource::PawnTowardsFocus) || (Source == EPG_AbilityTargetingSource::WeaponTowardsFocus)))
	{
...
void UPG_GameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData,
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (!bProjectileWeapon)
		{
			if (AController* Controller = GetControllerFromActorInfo())
			{
				...
				{
					// Confirm hit markers
					if (UPG_WeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UPG_WeaponStateComponent>())
					{
...
void UPG_GameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	UPG_WeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UPG_WeaponStateComponent>();
	...
	if (FoundHits.Num() > 0)
	{
		...
		{
			FCIS_GameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FCIS_GameplayAbilityTargetData_SingleTargetHit();
