...
UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_PG_Weapon_SteadyAimingCamera, "PG.Weapon.SteadyAimingCamera");


UPG_RangedWeaponInstance::UPG_RangedWeaponInstance(const FObjectInitializer& ObjectInitializer)
...
void UPG_RangedWeaponInstance::PostLoad()
{
	Super::PostLoad();
...
void UPG_RangedWeaponInstance::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
	UpdateDebugVisualization();
}

void UPG_RangedWeaponInstance::UpdateDebugVisualization()
{
...
void UPG_RangedWeaponInstance::Tick(float DeltaSeconds)
{
	APawn* Pawn = GetPawn();
	check(Pawn != nullptr);
...
void UPG_RangedWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UPG_RangedWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
}

void UPG_RangedWeaponInstance::AddSpread()
{
...
float UPG_RangedWeaponInstance::GetDistanceAttenuation(float Distance, const FGameplayTagContainer* SourceTags,
...
float UPG_RangedWeaponInstance::GetPhysicalMaterialAttenuation(const UPhysicalMaterial* PhysicalMaterial,
	const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
	float CombinedMultiplier = 1.0f;
	if (const UPG_PhysicalMaterialWithTags* PhysMatWithTags = Cast<const UPG_PhysicalMaterialWithTags>(PhysicalMaterial))
	{
...
void UPG_RangedWeaponInstance::ComputeSpreadRange(float& MinSpread, float& MaxSpread)
{
	HeatToSpreadCurve.GetRichCurveConst()->GetValueRange(/*out*/ MinSpread, /*out*/ MaxSpread);
}

void UPG_RangedWeaponInstance::ComputeHeatRange(float& MinHeat, float& MaxHeat)
{
	float Min1;
	float Max1;
...
bool UPG_RangedWeaponInstance::UpdateSpread(float DeltaSeconds)
{
...
bool UPG_RangedWeaponInstance::UpdateMultipliers(float DeltaSeconds)
{
	...
	if (const UCIS_CameraComponent* CameraComponent = UCIS_CameraComponent::FindCameraComponent(Pawn))
	{
		...
		AimingAlpha = (TopCameraTag == TAG_PG_Weapon_SteadyAimingCamera) ? TopCameraWeight : 0.0f;
	}
