...
#include "PG_GameplayAbility_FromEquipment.h"
#include "PG_GameplayAbility_RangedWeapon.generated.h"

class UPG_RangedWeaponInstance;
class APawn;

/** Defines where an ability starts its trace from and where it should face */
UENUM(BlueprintType)
enum class EPG_AbilityTargetingSource : uint8
{
...
/**
 * UCIS_GameplayAbility_RangedWeapon
 *
 * An ability granted by and associated with a ranged weapon instance
 */

UCLASS()
class UPG_GameplayAbility_RangedWeapon : public UPG_GameplayAbility_FromEquipment
{
	GENERATED_BODY()

public:

	UPG_GameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category="PG|Ability")
	UPG_RangedWeaponInstance* GetWeaponInstance() const;
...
protected:
	struct FRangedWeaponFiringInput
	{
		...
		UPG_RangedWeaponInstance* WeaponData = nullptr;
...
protected:
	...
	FTransform GetTargetingTransform(APawn* SourcePawn, EPG_AbilityTargetingSource Source) const;
