...
#include "Camera/CIS_CameraMode_ThirdPerson.h"

#include "Camera/CIS_CameraAssistInterface.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "GameFramework/Character.h"

namespace CIS_CameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UCIS_CameraMode_ThirdPerson::UCIS_CameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FCIS_PenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FCIS_PenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FCIS_PenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FCIS_PenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FCIS_PenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FCIS_PenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FCIS_PenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UCIS_CameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UCIS_CameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UCIS_CameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	ICIS_CameraAssistInterface* TargetControllerAssist = Cast<ICIS_CameraAssistInterface>(TargetController);

	ICIS_CameraAssistInterface* TargetActorAssist = Cast<ICIS_CameraAssistInterface>(TargetActor);
	...
	ICIS_CameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<ICIS_CameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		ICIS_CameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (ICIS_CameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UCIS_CameraMode_ThirdPerson::PreventCameraPenetration(AActor const& ViewTarget, FVector const& SafeLoc,
	FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
	...
	{
		FCIS_PenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(CIS_CameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UCIS_CameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UCIS_CameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UCIS_CameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
