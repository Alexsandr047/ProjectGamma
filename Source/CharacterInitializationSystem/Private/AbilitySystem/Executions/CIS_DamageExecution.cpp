...
#include "AbilitySystem/Executions/CIS_DamageExecution.h"
...
struct FDamageStatics
{
	...
	FDamageStatics()
	{
		HealthDef = FGameplayEffectAttributeCaptureDefinition(UCIS_HealthSet::GetHealthAttribute(), EGameplayEffectAttributeCaptureSource::Target, false);
		BaseDamageDef = FGameplayEffectAttributeCaptureDefinition(UCIS_CombatSet::GetBaseDamageAttribute(), EGameplayEffectAttributeCaptureSource::Source, true);
	}
};
...
UCIS_DamageExecution::UCIS_DamageExecution()
{
...
void UCIS_DamageExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams,
	FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
#if WITH_SERVER_CODE
	const FGameplayEffectSpec& Spec = ExecutionParams.GetOwningSpec();
	FCIS_GameplayEffectContext* TypedContext = FCIS_GameplayEffectContext::ExtractEffectContext(Spec.GetContext());
	check(TypedContext);
	...
	{
		DamageInteractionAllowedMultiplier = 1; //Dont use Team
		/*UCIS_TeamSubsystem* TeamSubsystem = HitActor->GetWorld()->GetSubsystem<UCIS_TeamSubsystem>();
	...
	if (const ICIS_AbilitySourceInterface* AbilitySource = TypedContext->GetAbilitySource())
	{
	...
	if (DamageDone > 0.0f)
	{
		OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(UCIS_HealthSet::GetHealthAttribute(), EGameplayModOp::Additive, -DamageDone));
	}
